...
app.post("/advocate", authenticateJWT, async (req, res) => {
  try {
    ...
    if (!hearingCourt || !advocateName) {
      return res
        .status(400)
        .json({ error: "Hearing court and advocate name are required" });
    }

    db.run(
      "INSERT INTO AdvocateForm (hearingCourt, advocateName, user_id) VALUES (?, ?, ?)",
      [hearingCourt, advocateName, userId],
      function (err) {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
...
app.get("/advocate", authenticateJWT, (req, res) => {
  ...
  db.all(
    "SELECT * FROM AdvocateForm WHERE user_id = ?",
    [userId],
    (err, forms) => {
      if (err) {
        return res.status(500).json({ error: err.message });
...
app.post("/afterproxy", authenticateJWT, async (req, res) => {
  try {
    ...
    db.run(
      "INSERT INTO AfterProxyForm (contactMethod, contactInfo, user_id) VALUES (?, ?, ?)",
      [contactMethod, contactInfo, userId],
      function (err) {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
...
app.get("/alerts", authenticateJWT, (req, res) => {
  const userId = req.user.id;
  db.all(
    "SELECT id,title, startDate, completionDate, assignTo FROM AlertsForm WHERE user_id = ?",
    [userId],
    (err, forms) => {
      if (err) {
        return res.status(500).json({ error: err.message });
...
app.delete("/calendar/alerts/:taskId", authenticateJWT, (req, res) => {
  ...
  db.run(
    "DELETE FROM AlertsForm WHERE id = ? AND user_id = ?",
    [taskId, userId],
    (err) => {
      if (err) {
        return res.status(500).json({ error: err.message });
...
app.delete("/calendar/hearings/:taskId", authenticateJWT, (req, res) => {
  ...
  db.run(
    "DELETE FROM CourtHearing WHERE id = ? AND user_id = ?",
    [taskId, userId],
    (err) => {
      if (err) {
        return res.status(500).json({ error: err.message });
...
app.get("/alerts/download-pdf/:alertId", authenticateJWT, async (req, res) => {
  try {
    ...
    pdf.create(htmlContent).toStream((err, stream) => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=Alert_${alertData.id}.pdf`
      );
...
app.get("/dashboard/teammemberform", authenticateJWT, (req, res) => {
  ...
  db.all(
    "SELECT id,fullName, email, designation,selectedGroup FROM TeamMembers WHERE user_id = ?",
    [userId],
    (err, forms) => {
      if (err) {
        return res.status(500).json({ error: err.message });
...
app.delete(
  "/dashboard/teammemberform/:memberId",
  authenticateJWT,
  async (req, res) => {
    try {
...
app.get(
  "/dashboard/teammemberform/download-pdf/:memberId",
  authenticateJWT,
  async (req, res) => {
    try {
      const { memberId } = req.params;
    ...
    `;
      ...
      pdf.create(htmlContent).toStream((err, stream) => {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename=TeamMember_${memberData.id}.pdf`
        );
...
app.get("/dashboard/groupform", authenticateJWT, (req, res) => {
  try {
    const userId = req.user.id;
    db.all(
      "SELECT groupName FROM GroupForm WHERE user_id = ?",
      [userId],
      (err, groupName) => {
        if (err) {
          console.error(err);
...
app.get("/dashboard/clientform", authenticateJWT, (req, res) => {
  try {
    const userId = req.user.id;
    db.all(
      "SELECT firstName FROM ClientForm WHERE user_id = ?",
      [userId],
      (err, firstName) => {
        if (err) {
          console.error(err);
...
app.post("/bill", authenticateJWT, async (req, res) => {
  try {
    ...
    if (!billNumber || !title) {
      return res
        .status(400)
        .json({ error: "Bill number and title are required" });
    }
    db.run(
...
app.get("/billdata", authenticateJWT, (req, res) => {
  const userId = req.user.id;
  db.all(
    "SELECT id,billNumber, title, dateFrom, dateTo, amount, totalAmountWithTax FROM BillForm WHERE user_id = ?",
    [userId],
    (err, forms) => {
      if (err) {
        return res.status(500).json({ error: err.message });
...
app.get("/billdata/download-pdf/:billId", authenticateJWT, async (req, res) => {
  ...
  <body>
    ...
    pdf.create(htmlContent).toStream((err, stream) => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=Bill_${billData.id}.pdf`
      );
...
app.get("/caseformdata", authenticateJWT, (req, res) => {
  try {
    const userId = req.user.id;
    db.all(
      "SELECT id,title,caseCode,client,honorableJudge, opponentPartyName FROM CasesForm WHERE user_id = ?",
      [userId],
      (err, forms) => {
        if (err) {
          console.error(err);
...
app.get("/teammemberform", authenticateJWT, (req, res) => {
  try {
    const userId = req.user.id;
    db.all(
      "SELECT fullName FROM TeamMembers WHERE user_id = ?",
      [userId],
      (err, fullName) => {
        if (err) {
          console.error(err);
...
app.get(
  "/dashboard/caseformdata/download-pdf/:caseId",
  authenticateJWT,
  async (req, res) => {
    try {
      const { caseId } = req.params;
    ...
    `;
      ...
      pdf.create(htmlContent).toStream((err, stream) => {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename=Case_${caseData.id}.pdf`
        );
...
app.post("/dashboard/clientform", authenticateJWT, async (req, res) => {
  try {
    ...
    if (!firstName || !email) {
      return res
        .status(400)
        .json({ error: "First Name and Email are required fields" });
    }
    const userId = req.user.id;
    const query = `
...
app.get("/clientformdata", authenticateJWT, (req, res) => {
  try {
    const userId = req.user.id;
    db.all(
      "SELECT id,firstName,email,mobileNo,assignAlerts FROM ClientForm WHERE user_id = ?",
      [userId],
      (err, forms) => {
        if (err) {
          console.error(err);
...
app.get(
  "/clientformdata/download-pdf/:clientId",
  authenticateJWT,
  async (req, res) => {
    try {
      const { clientId } = req.params;
    ...
    `;
      ...
      pdf.create(htmlContent).toStream((err, stream) => {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename=Client_${clientData.id}.pdf`
        );
...
app.get("/dashboard/alertsform", authenticateJWT, (req, res) => {
  try {
    const userId = req.user.id;
    db.all(
      "SELECT title FROM AlertsForm WHERE user_id = ?",
      [userId],
      (err, title) => {
        if (err) {
          console.error(err);
...
app.post("/dashboard/groupform", authenticateJWT, async (req, res) => {
  try {
    ...
    if (!groupName || !priority) {
      return res
        .status(400)
        .json({ error: "Group name and priority are required" });
    }

    db.run(
      "INSERT INTO GroupForm (groupName, company, priority, user_id) VALUES (?, ?, ?, ?)",
      [groupName, company, priority, userId],
      function (err) {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
...
app.post("/invoiceform", authenticateJWT, async (req, res) => {
  try {
    ...
    db.run(
      "INSERT INTO InvoicesForm (invoiceNumber, client, caseType, date, amount, taxType, taxPercentage, fullAddress, hearingDate, title, dateFrom, dateTo, expensesAmount, expensesTaxType, expensesTaxPercentage, expensesCumulativeAmount, addDoc, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [
        invoiceNumber,
        client,
        caseType,
...
app.get("/invoiceformdata", authenticateJWT, (req, res) => {
  ...
  db.all(
    "SELECT id,title, invoiceNumber , date, client,expensesCumulativeAmount FROM InvoicesForm WHERE user_id = ?",
    [userId],
    (err, forms) => {
      if (err) {
        return res.status(500).json({ error: err.message });
...
app.get(
  "/invoiceformdata/download-pdf/:invoiceId",
  authenticateJWT,
  async (req, res) => {
    try {
      const { invoiceId } = req.params;
  ...
  <!-- Add more fields as needed -->
    ...
    `;
      ...
      pdf.create(htmlContent).toStream((err, stream) => {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename=Invoice_${invoiceData.id}.pdf`
        );
...
app.get("/caseform", authenticateJWT, (req, res) => {
  try {
    const userId = req.user.id;
    db.all(
      "SELECT id, title, caseType FROM CasesForm WHERE user_id = ?",
      [userId],
      (err, cases) => {
        if (err) {
          console.error(err);
  ...
  try {
    ...
    if (!hearingCourt || !partyName || !caseYear) {
      return res
        .status(400)
        .json({
          error: "Hearing court, party name, and case year are required",
        });
    }

    db.run(
      "INSERT INTO PartyNameForm (hearingCourt, partyName, caseYear, user_id) VALUES (?, ?, ?, ?)",
      [hearingCourt, partyName, caseYear, userId],
      function (err) {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
...
app.post("/reviewdoc", authenticateJWT, async (req, res) => {
  try {
    ...
    if (!reviewMethod || !contactMethod) {
      return res
        .status(400)
        .json({ error: "Review method and contact method are required" });
    }

    db.run(
      "INSERT INTO ReviewDocForm (reviewMethod, contactMethod, file, text, mobileNo, user_id) VALUES (?, ?, ?, ?, ?, ?)",
      [reviewMethod, contactMethod, file, text, mobileNo, userId],
      function (err) {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
        return res.json({
          message: "Review document form submitted successfully",
        });
      }
    );
  } catch (error) {
                  ...
                  db.run(
                    ...
                    (err) => {
                      if (err) {
                        ...
                        console.log(
                          "New notification stored in NotificationTable."
                        );
                      }
                    }
                  );
...
// getting data from data base which is rended over frontend
...
app.post("/dashboard/user/accept-proxy/:proxyId", authenticateJWT, (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      ...
      if (this.changes === 0) {
        return res.status(404).json({ error: 'Proxy not found or already accepted' });
      }

      // The proxy has been accepted, now retrieve information to notify the creator.
      db.get(
        ...
        (err, row) => {
          ...
          // If row is undefined, it means no matching proxy was found. This should not happen though as we just updated it.
          if (!row) {
            return res.status(404).json({ error: 'Proxy not found' });
          }

          // Create a notification for the proxy creator.
          ...
          db.run(
            ...
            (err) => {
              ...
              // Only now, we can safely send the response back to the client.
...
app.get(
  "/dashboard/user/accepted-proxy-notifications",
  authenticateJWT,
  (req, res) => {
    const userId = req.user.id;
    const currentDate = new Date().toISOString();
    ...
    db.all(
      "SELECT message, expirationDate, id FROM NotificationTable WHERE userId = ? AND expirationDate > ?",
      [userId, currentDate],
      (err, rows) => {
        if (err) {
          console.error("Database error:", err);
        ...
        try {
          ...
          const notifications = rows.map((row) => {
            // return {message:row.message,id:row.id};
            return row.message;
          });
          console.log("proxy",notifications);
          return res.json(notifications);
...
app.delete(
  "/dashboard/user/notifications/:notificationId",
  authenticateJWT,
  (req, res) => {
    const userId = req.user.id;
