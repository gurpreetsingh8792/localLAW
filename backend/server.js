    ...
    (err, rows) => {
      ...
      try {
        const notifications = rows.map((row) => {
          ...
          return {
            id: id,
            message: `The proxy has been generated by ${fullName}. The date of hearing is ${dateOfHearing} in state ${zipStateProvince} City - ${city}`,
          };
        });
...
app.post(
  "/dashboard/user/accept-proxy/:proxyId",
  authenticateJWT,
  (req, res) => {
    const userId = req.user.id;
...
app.delete(
  "/dashboard/user/proxy-activity/:activityId",
  authenticateJWT,
  (req, res) => {
    const userId = req.user.id;
    ...
    db.get(
      ...
      (err, row) => {
        ...
        db.run(
          "DELETE FROM ProxyActivity WHERE id = ?",
          [activityId],
          (deleteErr) => {
            if (deleteErr) {
